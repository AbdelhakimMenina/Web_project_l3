...
$id=$_SESSION['user_id'];
 ...
 if(isset($_POST['nom_lang'])){
    ...
    $resultat= mysqli_query($conn, "INSERT INTO langue(name,id_user) values ('$langue',$id)");
...
if (isset($TabDonnees) && isset($TabDonnees["NomAuteur"])){// print ($TabDonnees["NomAuteur"]);
	foreach ($TabDonnees["NomAuteur"] as $num=>$NAuteur){
		...
		$FAuteur .=CaseFinLigne().'</tr>';
		print ($FAuteur);
        $prenom =  $TabDonnees["PrénomAuteur"][$num];
        $email  =  $TabDonnees["EmailAuteur"][$num];
        $inst   =  $TabDonnees["InstAuteur"][$num];
        $id=$_SESSION['user_id'];
	    $conn=mysqli_connect("localhost","pma","pma","app");

        $result= mysqli_query($conn, "INSERT INTO autheur(Nom,Prenom,Email,Institution,id_user) values ('$NAuteur','$prenom','$email','$inst', $id)");
...
print ('<a class="infobulle"> 
       <img src="logo/image-aide.png" height=32px alt=" ? " /> 
       <div>
');					
					if ($regles != "") {
